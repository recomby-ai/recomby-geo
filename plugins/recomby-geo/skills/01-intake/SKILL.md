...
name: 01-intake
description: >
  Build the GEO brand context for a client by ingesting provided materials
  (PDF/DOCX/PPTX/XLSX, URLs, raw notes) and conducting structured AI-perception
  + competitor + community research. Writes brand_context.json validated
  against schemas/brand_context.schema.json. Use when starting a new client
  (clients/<slug>/inputs/ has materials but no brand_context.json), or when
  brand_context.json is marked status=stale. Downstream consumers: 02-audit,
  03-gap, 04-content-brief, 05-production.
---
...
- Send each to multiple LLMs. Preferred mechanism: invoke 02-audit's tooling
  (`gego` or `geo-aeo-tracker` shim) for a quick scout; OR call `WebFetch`
  on chat.openai.com share links / Perplexity / Gemini if no API.
...
## Reference (拿来主义 — consult, do not copy wholesale)

- **`AI2HU/gego`** — for Layer 2 AI Perception Scan, use their cross-LLM
  query runner. License: check repo.
- **`danishashko/geo-aeo-tracker`** — same purpose, alternative impl.
- **Schema.org `Organization` / `Brand` / `Event`** — confirm Layer 1 field
  names align with public structured-data conventions; helps 06-distribution.
- **Forrester ICP template / ChiefMartec persona model** — reference for
  Layer 1 `target_customer` field completeness.
- **Princeton KDD 2024 GEO paper (`GEO-optim/GEO`)** — defines what a

...
name: 07-reaudit
description: >
  Re-run 02-audit, diff against the previous baseline, attribute movement
  to specific content/distribution actions. Outputs reaudit/round-N.json
  validated against attribution_diff.schema.json. This is the closing-of-
  loop artifact that proves (or disproves) what's actually working. Run
  monthly after the previous month's distribution actions have had time
  to be indexed.
---
...
- `AI2HU/gego` / `danishashko/geo-aeo-tracker` — same shim as 02-audit
  (just running again).
- Princeton KDD 2024 (`GEO-optim/GEO`) — empirical basis for "what kind
  of content moves visibility" used in `actions_without_impact`
  diagnosis.
